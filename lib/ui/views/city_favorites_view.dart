...
class _CityFavoritesViewState extends State<CityFavoritesView> {
    ...
    return WillPopScope(
        onWillPop: () {
          Navigator.of(context).pop(_selectedIndex >= 0 &&
                  (_cityFavoritsList?.isNotEmpty ?? false)
              ? CityInfo(
                  ...
                  countryCode:
                      _cityFavoritsList?[_selectedIndex].countryCode ?? '',
                  isFavorite:
                      _cityFavoritsList?[_selectedIndex].isFavorite ?? false,
                )
              : null);
          ...
          appBar: AppBar(
            ...
            actions: <Widget>[
              Padding(
                ...
                child: IconButton(
                  onPressed: _cityFavoritsList != null &&
                          (_cityFavoritsList?.length ?? 0) > 1
                      ? () async {
                          CityInfo? cityValue =
                              _cityFavoritsList?[_selectedIndex];
                          _sortedByAscending *= -1;
                          _cityFavoritsList?.sort((cityInfo1, cityInfo2) {
                            if (cityInfo1.nameDesc.isEmpty &&
                                cityInfo2.nameDesc.isEmpty)
                              return cityInfo1.nameDesc
                                      .compareTo(cityInfo2.nameDesc) *
                                  _sortedByAscending;
                            else
                              return cityInfo1.name.compareTo(cityInfo2.name) *
                                  _sortedByAscending;
                          });
                          //　ソート後の結果を保存する
                          if (cityValue != null) {
  ...
  Widget _buildBody(BuildContext context) {
    return (_cityFavoritsList?.isNotEmpty ?? false)
    ...
    return Column(children: <Widget>[
      Theme(
        ...
        child: CheckboxListTile(
          title: Text(
            _cityFavoritsList?[index].nameDesc != null &&
                    (_cityFavoritsList?[index].nameDesc.isNotEmpty ?? false)
  ...
  Future<void> _readFavoriteData() async {
    ...
    setState(() {
      ...
      _checkedStatusList = (_cityFavoritsList?.length ?? 0) > 0
          ? List.filled(_cityFavoritsList?.length ?? 0, false)
          : [];
      if (_selectedIndex >= 0) {
  ...
  Future<void> _saveFavoriteData({CityInfo? cityValue}) async {
    await CityUtil().saveCityListIntoPref(
        cityList: _cityFavoritsList, selectedItem: cityValue);
