...
class CitySelectorView extends StatefulWidget {
  ...
  CitySelectorView({required this.title, required this.cityItem});
...
class _CitySelectorViewState extends State<CitySelectorView> {
  ...
  late TextEditingController _cityNameTextController;
  late TextEditingController _countryCdTextController;
  late _CityInputMode _cityMode;
  late String _countryNameDesc;
  ...
  void initState() {
    ...
    _countryNameDesc =
        CityUtil().getCountryNameDesc(countryCode: _cityItem.countryCode);

    _cityNameTextController = TextEditingController(
        text: _cityItem.zip.isEmpty && _cityItem.zip.isNotEmpty
            ? _cityItem.zip
            : _cityItem.name);
    _countryCdTextController =
        TextEditingController(text: _cityItem.countryCode);

    _cityMode = _cityItem.zip.isEmpty && _cityItem.zip.isNotEmpty
        ? _CityInputMode.zip
        : _CityInputMode.cityName;
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        onWillPop: () async {
          ...
          Navigator.of(context).pop(CityInfo(
              zip: _cityMode == _CityInputMode.zip
                  ? _cityNameTextController.text
                  : '',
              name: _cityMode == _CityInputMode.cityName
                  ? _cityNameTextController.text
                  : '',
              countryCode: _countryCdTextController.text,
          ...
          children: <Widget>[
            ...
            Radio(
              ...
              onChanged: (_CityInputMode? value) {
                FocusScope.of(context).unfocus();
                setState(() {
                  _cityMode = value ?? _CityInputMode.cityName;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Text(
                ...
                maxLines: null,
                style: TextStyle(
                    fontSize: 12.0, height: 0.9, color: Colors.black54),
              ),
            ),
          ],
        ),
        Row(
          children: [
            ...
            Container(
              ...
              child: Text(
                " ※郵便番号・市/町/村名の入力は日本以外の場合も可能です。いずれも国名コードが必要です。\n例： New York,US",
                maxLines: null,
                style: TextStyle(
                    fontSize: 12.0, height: 1.0, color: Colors.black54),
              ),
            ),
          ],
        ),
        Row(
          children: <Widget>[
            ...
            Container(
              width: MediaQuery.of(context).size.width - 160,
              child: TextFormField(
                  keyboardType: _cityMode == _CityInputMode.zip
                      ? TextInputType.numberWithOptions(signed: true)
                      : TextInputType.text,
                  inputFormatters: _cityMode == _CityInputMode.zip
                      ? <TextInputFormatter>[
                  ...
                  decoration: InputDecoration(
                      hintText: _cityMode == _CityInputMode.cityName
                          ? 'Chiyodaーku'
                          : '123-3456')),
            ),
            Text(' '),
              ...
              child: GestureDetector(
                ...
                child: AbsorbPointer(
                  child: TextFormField(
                      ...
                      autofocus: true,
                      decoration:
                          InputDecoration(counterText: '', hintText: 'JP')),
                ),
              ),
            ),
          ],
          ...
          children: <Widget>[
            ...
            Checkbox(
              ...
              onChanged: (bool? value) {
                setState(() {
                  _cityItem.isFavorite = value ?? false;
                });
              },
            ),
            Container(
  ...
  void _countryCodeTextDidChange() {
    Navigator.pushNamed(context, ScreenRouteName.selectCountry.name ?? '',
            arguments: {'countryCode': _countryCdTextController.text})
        .then((value) async {
      CountryInfo? countryValue = value as CountryInfo?;
