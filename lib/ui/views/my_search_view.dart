...
class _MySearchViewState extends State<MySearchView> with SingleTickerProviderStateMixin {
  ...
  static const List<String> _kSearchedButtonTextList = ["調剤日", "薬名", "飲み方", "服用回数", "医療機関", "医師名"];
  static const List<String> _kSearchedButtonHintList = ["yyyy/MM/dd", "くすり名を入力", "選択...", "服用回数を入力", "医療機関名を入力", "医師名を入力"];

  // Private variables
  SearchStatus _searchStatus = SearchStatus.ready;
  int _selectedTypeIndex = 0;
  int _selectDrogoUsageItemValue;
  Map<int, String>  _selectedEtcItem;
          ...
          Padding(
              ...
              child: Row(
                children: <Widget>[
                  ...
                  SearchType.dispeningDate == SearchType.values[_selectedTypeIndex] ? _showDatePickerWithTextField(context, DateTime.now()) :
                    (SearchType.usage == SearchType.values[_selectedTypeIndex] ? _buildSearchedItemDrogoUsage() : _buildSearchedItemDrogoEtc()),
                  // 検索ボタン
                  Container(
                    width: 35,
                    child: IconButton(
  ...
  Widget _buildSearchResultView() {
    ...
    if(param != null) {
      return MySearchResultView(searchedParam: param, searchedType: SearchType.values[_selectedTypeIndex],);
    } else {
      return Container();
    }
  }
  ...
  DrogoSearchParam _makeSearchParam() {
    ...
    switch(SearchType.values[_selectedTypeIndex]) {
      case SearchType.dispeningDate: //調剤日
        param = (_dateController.value.text != null && _dateController.value.text.isNotEmpty) ? DrogoSearchParam(dispeningDate: _dateController.value.text) : null;
        break;
      case SearchType.usage: //飲み方
        param = (_selectDrogoUsageItemValue != null) ? DrogoSearchParam(usage: _selectDrogoUsageItemValue) : null;
        break;
      case SearchType.drogoName: //薬名
        param = (_selectedEtcItem != null && _selectedEtcItem.isNotEmpty) ? DrogoSearchParam(drogoName: _selectedEtcItem[_selectedTypeIndex]) : null;
        break;
      case SearchType.times: //使用回数
        param = (_selectedEtcItem != null && _selectedEtcItem.isNotEmpty) ? DrogoSearchParam(times: int.tryParse(_selectedEtcItem[_selectedTypeIndex])): null;
        break;
      case SearchType.medicalInstituteName: //医療機関
        param = (_selectedEtcItem != null && _selectedEtcItem.isNotEmpty) ? DrogoSearchParam(medicalInstituteName: _selectedEtcItem[_selectedTypeIndex]) : null;
        break;
      case SearchType.doctorName: //医師名
        param = (_selectedEtcItem != null && _selectedEtcItem.isNotEmpty) ? DrogoSearchParam(doctorName: _selectedEtcItem[_selectedTypeIndex]) : null;
        break;
      default:
        param = null;
        break;
      ...
      Container(
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownButton<int>(
            ...
            items: List<DropdownMenuItem<int>>.generate(_kSearchedFieldLength, (index) => DropdownMenuItem(
                child: Text(
            ...
            onChanged: (int value) {
              setState(() {
                ...
                //　検索種別が調剤日と飲み方以外の場合
                if (![SearchType.dispeningDate, SearchType.usage].contains(SearchType.values[_selectedTypeIndex])) {
                  _textController.text = _selectedEtcItem[_selectedTypeIndex];
                }
              });
            },
          ),
        ...
        child:GestureDetector(
          onTap: () => {
              DatePicker.showDatePicker(
                ...
                onConfirm2: (dateTime, selectedIndexList) {
                  setState(() {
                    _dateController.value = TextEditingValue(text: DateFormat('yyyy/MM/dd').format(dateTime));
                    _searchStatus = SearchStatus.ready;
                  });
                },
              )                           
      ...
      Container(
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownButton<int>(
            ...
            items: List<DropdownMenuItem<int>>.generate(DrogoUsages.values.length, (codeIdx) => DropdownMenuItem(
                child: Text(
            ...
            onChanged: (int value) {
              setState(() {
                _selectDrogoUsageItemValue = value;
                _searchStatus = SearchStatus.ready;
              });
            },
          ),
