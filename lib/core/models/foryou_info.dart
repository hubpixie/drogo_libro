...
class ForyouInfo {
  ...
  ForyouInfo(
      ...
      this.userId,
      this.bloodType,
      this.medicalHistoryTypeList,
      this.medicalHdText,
      this.medicalEtcText,
      this.allergyHistoryTypeList,
      this.allergyEtcText,
      this.suplementTypeList,
      this.suplementEtcText,
      this.sideEffectList});
  ...
  ForyouInfo.fromJson(Map<String, dynamic> json) {
    ...
    medicalHistoryTypeList = () {
      ...
      return dsList != null
          ? dsList.map((element) => element != 0).toList()
          : <bool>[];
    }();
    ...
    sideEffectList = () {
      ...
      if (dsList != null &&
          dsList.isNotEmpty &&
          dsList.length > SideEffectInfo.kListMxLength) {
        dsList = dsList.take(SideEffectInfo.kListMxLength).toList();
      }
      return dsList != null && dsList.isNotEmpty
          ? dsList.map((element) => SideEffectInfo.fromJson(element)).toList()
          : <SideEffectInfo>[];
    }();
  }
  ...
  Map<String, dynamic> toJson() {
    ...
    data['medical_history_type_list'] =
        this.medicalHistoryTypeList?.map((element) => element ? 1 : 0).toList();
    data['medical_hd_text'] = this.medicalHdText;
    data['medical_etc_text'] = this.medicalEtcText;
    data['allergy_history_type_list'] =
        this.allergyHistoryTypeList?.map((element) => element ? 1 : 0).toList();
    data['allergy_etc_text'] = this.allergyEtcText;
    data['suplement_type_list'] =
        this.suplementTypeList?.map((element) => element ? 1 : 0).toList();
    data['suplement_etc_text'] = this.suplementEtcText;
    data['side_effect_list'] =
        this.sideEffectList?.map((element) => element.toJson()).toList();
