...
class WeatherViewModel extends BaseViewModel {
  WeatherInfosService? _weatherService =
      ServiceSetting.locator<WeatherInfosService>();
  List<WeatherInfo> _hourlyForecastData = [];
  List<WeatherInfo> _weeklyForecastData = [];

  DataResult? get fetchedCityNameFromLocation =>
      _weatherService?.fetchedCityNameFromLocation;
  ...
  /// Get city name
  ///
  Future getCityNameFromLocationuserId(
      {double latitude = 0, double longitude = 0}) async {
    setState(ViewState.Busy);
    await _weatherService?.getCityNameFromLocation(
  ...
  Future getWeatherData(
      {@required CityInfo? cityParam, bool forecastEnabled = false}) async {
    setState(ViewState.Busy);
    await _weatherService?.getWeatherData(cityParam: cityParam);
    WeatherInfo? todayWeather = _weatherService?.fetchedWeatherInfo.result;
    if ((_weatherService?.fetchedWeatherInfo.hasData ?? false) &&
        cityParam != null) {
