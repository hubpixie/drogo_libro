...
class CityUtil {
  ...
  late int _selectedFavoriteIndex;
  CityInfo? _savedCity;
  late List<CityInfo> _cityFavoriteList;
  late List<CityInfo> _romajiCityList;
  ...
  String getCountryNameDesc({@required String? countryCode}) {
    ...
    try {
      cinfo =
          _countryNameList.firstWhere((element) => element.code == countryCode);
    } catch (error) {}
    return cinfo?.jpName ?? (countryCode ?? '');
  }

  Future<void> readCityInfoFromPref({bool isDefault = false}) async {
    ...
    if (selectedCityIdx < 0) {
      ...
      if (citiesjson != null) {
        ...
        _cityFavoriteList =
            parsed.map((element) => CityInfo.fromJson(element)).toList();
        _savedCity = selectedCityIdx < _cityFavoriteList.length
            ? _cityFavoriteList[selectedCityIdx]
            : _cityFavoriteList[0];
      }
    }
    _savedCity = (_savedCity == null && isDefault) ? CityInfo() : _savedCity;
  }
  ...
  Future<void> readFavoriteData() async {
    ...
    if (citiesjson != null) {
      var parsed = json.decode(citiesjson) as List<dynamic>;
      _cityFavoriteList = parsed.isNotEmpty
          ? parsed.map((element) => CityInfo.fromJson(element)).toList()
          : [];
    } else {
      _cityFavoriteList = [];
  ...
  Future<void> saveCityInfoIntoPref({CityInfo? cityValue}) async {
    ...
    if (cityValue != null) {
      ...
      // パラメータをcityValueカレントCityにセット
      _selectedFavoriteIndex = foundIdx < 0 ? cityList.length - 1 : foundIdx;
  ...
  Future<void> saveCityListIntoPref(
      {List<CityInfo>? cityList, CityInfo? selectedItem}) async {
    final prefs = await SharedPreferences.getInstance();
    if (cityList != null) {
      // 追加後のCityListをJson化し、保存する
      final savedJson = cityList.map((element) => element.toJson()).toList();
      ...
      _selectedFavoriteIndex = cityList.indexWhere((element) =>
          (((element.name.isNotEmpty && element.name == selectedItem?.name) ||
                  (element.zip.isNotEmpty &&
                      element.zip == selectedItem?.zip)) &&
  ...
  Future<void> loadRomajiCityCsv() async {
    final myData =
        await rootBundle.loadString('assets/csv/romaji-chimei-all-u.csv');
    List<List<dynamic>> list =
        CsvToListConverter().convert(myData, fieldDelimiter: ',', eol: '\n');
    _romajiCityList = list
        .map((element) => CityInfo(
              name: element[3],
              nameDesc: element[0],
            ))
        .toList();
  ...
  Future<void> loadCountryNameCsv() async {
    final myData =
        await rootBundle.loadString('assets/csv/country_name_list.csv');
    List<List<dynamic>> list = CsvToListConverter()
        .convert(myData, fieldDelimiter: ',', eol: '\r\n')
        .sublist(1);
    _countryNameList = list
        .map((element) => CountryInfo(
